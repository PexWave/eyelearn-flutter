...
class NounPractice extends StatefulWidget {
  const NounPractice({ Key? key }) : super(key: key);
...
class _NounPracticeState extends State<NounPractice> with RouteAware {
...
Future<void> getlesson() async {
  ...
  try {
      final response = await http.get(Uri.parse("${apiUrl}/api/practices/grammar/practice-grammar-noun"));
  ...
  if (response.statusCode == 200) {
      ...
      // print(items);
      await playaudio('navigationguide/intonation_instructions.mp3');
      await playaudio('navigationguide/after_intro.mp3');
